...
class UltraDetailedAnalysisEngine:
    ...
    def _execute_avatar_analysis(self, data: Dict[str, Any], research_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if json_text:
            try:
                ...
                # Validação básica com nome alternativo
                required_fields = ['nome_ficticio', 'idade', 'perfil_demografico']
                
                # Se não tem nome_ficticio, tenta 'nome' ou cria um
                if isinstance(avatar_data, dict):
                    if 'nome_ficticio' not in avatar_data and 'nome' in avatar_data:
                        avatar_data['nome_ficticio'] = avatar_data['nome']
                    elif 'nome_ficticio' not in avatar_data:
                        avatar_data['nome_ficticio'] = f"Avatar {segmento}"
                
                if isinstance(avatar_data, dict) and all(field in avatar_data for field in required_fields):
                    ...
                    missing = [f for f in required_fields if f not in avatar_data] if isinstance(avatar_data, dict) else required_fields
                    logger.warning(f"⚠️ Campo obrigatório ausente no avatar: {', '.join(missing)}")

            except json.JSONDecodeError as e:
                logger.warning(f"❌ JSON inválido encontrado na resposta: {e}")
            except Exception as e:
                logger.warning(f"⚠️ Erro ao processar dados do avatar: {e}")
        else:
        ...
        avatar_data = {
            "nome_ficticio": f"Avatar {segmento}",
            "idade": "25-45 anos",
            "perfil_demografico": f"Profissional de {segmento}",
        ...
        for line in lines:
            ...
            if 'nome' in line.lower() and ':' in line:
                ...
                if nome and len(nome) < 50:
                    avatar_data['nome_ficticio'] = nome
            elif 'idade' in line.lower() and ':' in line:
                idade = line.split(':')[1].strip()
                if idade and len(idade) < 20:
                    avatar_data['idade'] = idade
            elif 'profissao' in line.lower() and ':' in line:
                profissao = line.split(':')[1].strip()
                if profissao and len(profissao) < 100:
                    avatar_data['perfil_demografico'] = profissao
            ...
            elif 'objeção' in line.lower() or 'dificuldade' in line.lower():
                if ':' in line:
                    objecao = line.split(':')[1].strip()
                    if objecao and len(objecao) < 100:
                        avatar_data['objecoes'].append(objecao)
    ...
    # Métodos faltantes que foram referenciados no código original mas não implementados
    # Correção: Adicionar stubs/implementações para métodos chamados mas não definidos

    def _execute_competitor_analysis(self, data: Dict[str, Any], avatar_data: Dict[str, Any]) -> Dict[str, Any]:
        """Executa análise de concorrência - OBRIGATÓRIA (Stub)"""
        logger.warning("Competitor Analysis não implementado. Retornando dados de fallback.")
        return {
            "analise_concorrentes": [
                {"nome": "Concorrente A (Fallback)", "url": "http://example.com/concorrenteA", "pontos_fortes": ["Preço", "Marca"]},
                {"nome": "Concorrente B (Fallback)", "url": "http://example.com/concorrenteB", "pontos_fortes": ["Inovação", "Suporte"]}
            ],
            "metadata_competitor_analysis": {"fallback_mode": True}
        }

    def _execute_sales_funnel_optimization(self, avatar_data: Dict[str, Any], data: Dict[str, Any]) -> Dict[str, Any]:
        """Executa otimização de funil de vendas - OBRIGATÓRIO (Stub)"""
        logger.warning("Sales Funnel Optimization não implementado. Retornando dados de fallback.")
        return {
            "funil_otimizado": [
                {"etapa": "Conscientização (Fallback)", "estrategia": "Marketing de Conteúdo"},
                {"etapa": "Consideração (Fallback)", "estrategia": "Webinars e Demonstrações"},
                {"etapa": "Decisão (Fallback)", "estrategia": "Depoimentos e Estudos de Caso"}
            ],
            "metadata_sales_funnel_optimization": {"fallback_mode": True}
        }

    def _execute_strategic_positioning(self, avatar_data: Dict[str, Any], data: Dict[str, Any]) -> Dict[str, Any]:
        """Executa posicionamento estratégico - OBRIGATÓRIO (Stub)"""
        logger.warning("Strategic Positioning não implementado. Retornando dados de fallback.")
        return {
            "posicionamento": {
                "proposta_unica_valor": "Solução completa para [problema do cliente] (Fallback)",
                "diferenciais": ["Tecnologia avançada", "Suporte dedicado"],
                "mensagem_chave": "Transforme seu negócio com [produto/serviço] (Fallback)"
            },
            "metadata_strategic_positioning": {"fallback_mode": True}
        }

    def _execute_strategic_action_plan(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Executa plano de ação estratégico - OBRIGATÓRIO (Stub)"""
        logger.warning("Strategic Action Plan não implementado. Retornando dados de fallback.")
        return {
            "plano_acao": [
                {"acao": "Pesquisa de mercado aprofundada (Fallback)", "prazo": "1 semana", "responsavel": "Marketing"},
                {"acao": "Desenvolvimento de campanha (Fallback)", "prazo": "2 semanas", "responsavel": "Marketing"}
            ],
            "metadata_strategic_action_plan": {"fallback_mode": True}
        }

    def _execute_keywords_analysis(self, avatar_data: Dict[str, Any], research_data: Dict[str, Any], data: Dict[str, Any]) -> Dict[str, Any]:
        """Executa análise de palavras-chave - OBRIGATÓRIA (Stub)"""
        logger.warning("Keywords Analysis não implementado. Retornando dados de fallback.")
        return {
            "palavras_chave": [
                {"keyword": "exemplo keyword 1 (Fallback)", "volume": 1000, "competicao": "baixa"},
                {"keyword": "exemplo keyword 2 (Fallback)", "volume": 500, "competicao": "media"}
            ],
            "metadata_keywords_analysis": {"fallback_mode": True}
        }
